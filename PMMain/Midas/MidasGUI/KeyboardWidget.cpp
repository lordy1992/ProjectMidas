    ...
    selectedKey(-1)
{
   ...
   // setPalette(pal);
    ...
    outerSelected = true;
 /*   ringData wheel1;
    wheel1.outerRing.push_back(keyboardValue('a', 'A'));
    wheel1.outerRing.push_back(keyboardValue('b', 'B'));
    wheel1.outerRing.push_back(keyboardValue('c', 'C'));
    wheel1.outerRing.push_back(keyboardValue('d', 'D'));
    wheel1.outerRing.push_back(keyboardValue('e', 'E'));
    wheel1.outerRing.push_back(keyboardValue('f', 'F'));
    wheel1.outerRing.push_back(keyboardValue('g', 'G'));
    wheel1.outerRing.push_back(keyboardValue('h', 'H'));
    wheel1.outerRing.push_back(keyboardValue('i', 'I'));
    wheel1.outerRing.push_back(keyboardValue('j', 'J'));
    wheel1.outerRing.push_back(keyboardValue('k', 'K'));
    wheel1.outerRing.push_back(keyboardValue('l', 'L'));
    wheel1.outerRing.push_back(keyboardValue('m', 'M'));

    wheel1.innerRing.push_back(keyboardValue('n', 'N'));
    wheel1.innerRing.push_back(keyboardValue('o', 'O'));
    wheel1.innerRing.push_back(keyboardValue('p', 'P'));
    wheel1.innerRing.push_back(keyboardValue('q', 'Q'));
    wheel1.innerRing.push_back(keyboardValue('r', 'R'));
    wheel1.innerRing.push_back(keyboardValue('s', 'S'));
    wheel1.innerRing.push_back(keyboardValue('t', 'T'));
    wheel1.innerRing.push_back(keyboardValue('u', 'U'));
    wheel1.innerRing.push_back(keyboardValue('v', 'V'));
    wheel1.innerRing.push_back(keyboardValue('w', 'W'));
    wheel1.innerRing.push_back(keyboardValue('x', 'X'));
    wheel1.innerRing.push_back(keyboardValue('y', 'Y'));
    wheel1.innerRing.push_back(keyboardValue('z', 'Z'));
...
int KeyboardWidget::getSelectedKeyFromAngle(double angle)
{
    ...
    return (int)((angle - startAngle) / deltaAngle);
}
...
void KeyboardWidget::paintEvent(QPaintEvent *event)
{
    ...
    if (selectedWheel >= 0)
    {
        std::vector<keyboardValue> *innerRing, *outerRing;
...
void KeyboardWidget::drawRing(QPainter &painter, std::vector<keyboardValue> *ring, int ringInnerRad, bool isSelected)
{
    qreal deltaAngle = 360.0 / ring->size();
    qreal startAngle = 90.0 + deltaAngle / 2;
    std::vector<keyboardValue>::iterator it;
